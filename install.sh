...
main() {
  if [[ "${1:-}" == --help ]]; then
    echo 'Usage: sudo bash install.sh [--help] (installs main into /opt/deal-onboarding-src)'
    exit 0
  fi
  ...
  local src="${DEAL_ONBOARDING_SRC_DIR:-/opt/deal-onboarding-src}"
  if [[ -e "$src" ]]; then
    echo "$src already exists. Use deal-onboarding update, or run its scripts/bootstrap.sh to reconfigure." >&2
    exit 1
  fi
  dnf install -y git ca-certificates
  git clone --branch main --single-branch https://github.com/ElcanoTek/deal-onboarding.git "$src"
